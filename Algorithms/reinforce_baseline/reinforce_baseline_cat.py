...
Original file is located at
    https://colab.research.google.com/drive/1r_uiIiKFEuriiAI35Tg5WA4WckUpfHKf
"""
...
class ReinforceAgent():
    ...
    def generate_episode(self):
        ...
        episode = []
        # state = self.env.d0_start_state()
        state = self.env.S_0
        t=0

        while not self.env.is_terminal(state) and t < 5000:
    ...
    def update(self,episode):
        ...
        for t in range(T):
            ...
            G_t = sum([self.env.gamma ** (i-t) * episode[i][2] for i in range(t, T)])
            G_t_tensor = torch.tensor([G_t], dtype=torch.float32)
            # G_t_tensor = torch.tensor([G_t], dtype=torch.float32).unsqueeze(0)
            ...
            action_probs = self.policy_network(state_tensor)
            # print(episode[t][1],action_probs)

            # log_prob_action = torch.log(action_probs[0,episode[t][1]])
            log_prob_action = torch.log(action_probs[0, episode[t][1]] + 1e-9)
...
def main():
    ...
    # plot_avg_rewards(all_rewards, title="REINFORCE with Baseline for CatMonsterWorld (5 Runs)")
    # # Initialize agent
    # agent = ReinforceAgent(env, state_size, action_size, lr_policy, lr_value)
    #
    # # Train agent
    ...
    #
    # # Plot results
    # plot_rewards(rewards, title="REINFORCE with Baseline for CatMonsterWorld")
